...
jtArrangeColsClass <- if (requireNamespace("jmvcore", quietly = TRUE)) R6::R6Class(
    ...
    private = list(
        ...
        .chkVar = function() {
            (length(self$options$varOrd) >= 2 && dim(self$readDataset())[1] >= 1)
        },
        ...
        .crrArg = function() {
            list(dtaInp = self$readDataset(), varOrd = unique(c(self$options$varOrd, rep(self$options$varAll, self$options$blnAll))))
        },

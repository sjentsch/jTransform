    ...
    private = list(
        ...
        .run = function() {
            ...
            if (nzchar(srcTrm) && dim(self$data)[2] > 1) {
                ...
                crrArg <- list(dtaInp = self$data, srcTrm = srcTrm, whlTrm = self$options$whlTrm, incCmp = self$options$incCmp,
                               incRcd = self$options$incRcd, incID  = self$options$incID,  incNom = self$options$incNom,
                               incOrd = self$options$incOrd, incNum = self$options$incNum)
                ...
                if (length(srcRes) > 0) {
                    outRes <- paste0(c(sprintf("Value “%s” (%s) found in...\n", srcTrm, ifelse(crrArg$whlTrm, "exact match", "partial or exact match")),
                                       paste0(names(srcRes), ": ", sapply(srcRes, paste, collapse = ", "))), collapse = "\n")

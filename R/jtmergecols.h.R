    ...
    public=list(
        initialize=function(options) {
            ...
            self$add(jmvcore::Html$new(
                ...
                title="",
                refs="jmvReadWrite",
                content="<b>Adds columns from other jamovi data file(s) to the current data set (matching them by one or more ID variables).</b><br> Assign one or more variables that appear in all data sets (e.g., with a participant code) to \u201CVariable(s) to match the data sets by\u201D. Afterwards, you need to assign the remaining variables (i.e., those that you do not want to exclude from the output data file) to \u201CFurther variables in the output\u201D.<br> The file browser under \u201CData set(s) to add\u201D represents a \u201Ctweaked\u201D solution: Because it only delivers the file name back, the home directory is afterwards searched for those file name(s). If a file can't be located in the home directory, or if it exists in more than one locations inside the home directory, an error is returned. Although it is less convenient, manually providing the file name including the path may therefore be both quicker and less error-prone. If several file names are provided, they have to be separated by semicolons.<br> There are four different types of merging operations: The first option keeps all cases (but columns in the resulting data set may be empty if they did not contain values in same input data sets), the second option keeps only those cases where all datasets contain the same value in the matching variable, for \"left\" all cases from the first data set in fleInp are kept (whereas cases that are only contained in input data set two or higher are dropped), for \"right\" all cases from the second (or any higher) data set in fleInp are kept.<br> Under \u201COutput file\u201D, you can adjust the name of the output file. You may also add a directory to the file name. If no path is given, the output file is stored in the home directory.<br>\n"))
            self$add(jmvcore::Html$new(
                options=options,
                name="txtOut",
                title="",
